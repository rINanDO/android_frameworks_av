...
CameraClient::CameraClient(const sp<CameraService>& cameraService,
        ...
        Client(cameraService, cameraClient, attributionAndPermissionUtils, 
                clientPackageName, systemNativeClient, clientFeatureId, cameraIdStr, api1CameraId,
                cameraFacing, sensorOrientation, clientPid, clientUid, servicePid, rotationOverride)
{
    int callingPid = getCallingPid();
    LOG1("CameraClient::CameraClient E (pid %d, id %d)", callingPid, api1CameraId);
    ...
    LOG1("CameraClient::CameraClient X (pid %d, id %d)", callingPid, api1CameraId);
}
...
binder::Status CameraClient::disconnect() {
    ...
    */
    ...
    mCameraServiceProxyWrapper->logClose(mCameraIdStr, closeLatencyMs, false);
    return res;
}
...
status_t CameraClient::setParameters(const String8& params) {
    LOG1("setParameters (pid %d) (%s)", getCallingPid(), params.c_str());
...
String8 CameraClient::getParameters() const {
    ...
    if (getCallingPid() != mServicePid && checkPidAndHardware() != NO_ERROR) {
        return String8();
    }

    std::string params(mHardware->getParameters().flatten());
    LOG1("getParameters (pid %d) (%s)", getCallingPid(), params.c_str());
    return String8(params.c_str());
}
...
sp<CameraClient> CameraClient::getClientFromCookie(void* user) {
    std::string cameraId = String8::format("%d", (int)(intptr_t) user).c_str();
...
status_t CameraClient::setAutoframingOverride(uint8_t /*autoframingMode*/)
{
    return OK;
}
...
status_t CameraClient::injectSessionParams(const CameraMetadata& /*sessionParams*/) {
    return OK;
}
